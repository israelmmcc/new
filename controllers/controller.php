...
class MvcController{
	...
	public function vistaMateriasController(){
		...
		echo'<tr>
				...
				<td><a href="index.php?action=agregar_alumno&id='.$item["id"].'&idcarrera='.$nombreCarrera["id"].'"><button class="small success">Agregar Alumno</button></a></td>

                <td><a href="index.php?action=editar_materia&id='.$item["id"].'"><button class="small warning">Editar</button></a></td>

				<td><a href="index.php?action=intMaterias&idBorrar='.$item["id"].'"><button class="small alert">Borrar</button></a></td>
			</tr>';
		}
	}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	#------------------------------------
	#Permite hacer un registro de una materia en la bd.
		public function registroAlumnoXMateriaController(){
			    
			$datosController = array( "matricula"=>$_GET["matricula"], 
								      "idmateria"=>$_GET["id"]
								  );
			$respuesta = Datos::registroAlumnoXMateriaModel($datosController);


			if($respuesta == "success"){
			header('location:index.php?action=agregar_alumno&id='.$_GET["id"].'&idcarrera='.$_GET["idcarrera"].'');
			}
			else{
			header("location:index.php");
			}
	...
	public function borrarAlumnoXMateriaController(){
			...
			$respuesta = Datos::borrarAlumnoXMateriaModel($datosController);
...
///////////////////////EDITAR GRUPO BASE CONTROLLER
	public function editarBaseGruposController(){

        $id=$_GET["id"];
		$respuesta_carreras = Datos::obtenerCarrerasModel("carrera");
		$respuesta_grupo = Datos::obtenerGrupo($id);
		...
		echo'<label for="num_grupo">Nombre:</label>
			 <input type="text" name="id" id="num_grupo" value='.$respuesta_grupo["id"].' hidden>
	...
	public function editarGruposController(){
		...
		if(isset($_POST["id"])){
			...
			$respuesta = Datos::actualizarGrupoModel($datosController, "grupos");
			 ...
			 if($respuesta == "success"){
			 	header("location:index.php?action=intGrupos");
			 }
			 else{
			 	header("location:index.php");
			 	echo "ENTRA";
...
////////////////////////EDITAR MATERUAS BASE CONTROLLER
public function editarBaseMateriasController(){

        $id=$_GET["id"];
		$respuesta_carreras = Datos::obtenerCarrerasModel("carrera");
		$respuesta_maestros = Datos::obtenerMaestrosModel("maestros");
		$respuesta_materia = Datos::obtenerMateria($id);
		...
		for($i=0;$i<sizeof($respuesta_maestros);$i++)
			$st_maestros=$st_maestros."<option value='".$respuesta_maestros[$i]['num_empleado']."'>".$respuesta_maestros[$i]['nombre']."</option>";

		echo'<label for="num_empleado">Nombre:</label>
			 <input type="text" name="id" id="num_empleado" value='.$respuesta_materia["nombre"].'>
			 <label for="nombre">Nombre Materia:</label>
			 <input type="text" name="nombre" required id="nombre" value='.$respuesta_materia["nombre"].'>
			 <label for="id_carrera">Carrera:</label>
			 <select id="carreras" class="js-example-basic-multiple" name="id_carrera">
				  '.$st_carreras.'
			 </select>
			 ...
			 <hr>
			 <label for="num_maestro">Maestros:</label>

			 <select id="num_maestro" class="js-example-basic-multiple" name="num_maestro">
				  '.$st_maestros.'
			 </select>
	...
	public function editarMateriasController(){
		...
		if(isset($_POST["nombre"])){
			...
			$respuesta = Datos::actualizarMateriaModel($datosController, "materias");
			 ...
			 if($respuesta == "success"){
			 	header("location:index.php?action=intMaterias");
			 }
			 else{
			 	header("location:index.php");
			 }
		...
		if(isset($_COOKIE['nivel'])){
			if($_COOKIE['nivel']=="1")
				echo'
					...
					<li><a href="index.php?action=intGrupos">Grupos</a></li>
					<li><a href="index.php?action=intMaterias">Materias</a></li>
	...
	/////////////////////////////REGISTRO BASE MATERIASS



		public function registroBaseMateriasController(){
//Array de datos de maestros y carrera
		$respuesta_carreras = Datos::obtenerCarrerasModel("carrera");
		$respuesta_maestros = Datos::obtenerMaestrosModel("maestros");
...
$st_maestros="";
		...
		echo'<label for="num_empleado">Identificador:</label>
			 <input type="text" name="id" id="num_empleado">
			 <label for="nombre">Nombre Materia:</label>
...
//Array de datos de maestros y carrera
		...
		echo'<label for="num_grupo">Identificador:</label>
			 <input type="text" name="id" id="num_grupo">
    ...
    #Registro de grupos
	#------------------------------------
	#Permite hacer un registro de un grupo en la bd.
	...
	#------------------------------------
	#Se encarga de controlar la vista de la interfaz de alumnos
	public function vistaAgregarAlumnoController(){
		$id = $_GET["id"];
		$idcarrera = $_GET["idcarrera"];
